                ...
                <tbody>
                    @foreach ($facturas as $factura)
                        <tr>
                            ...
                            <td>
                                {!! HTML::link('#', '<i class="icon-file-empty" aria-hidden="true"></i>', [ 'class' => 'btn btn-default envio-facturas', 'data-toggle' => 'tooltip', 'data-placement' => 'bottom', 'data-original-title' => 'Envio por email', 'data-id' => $factura->getId() ]) !!}
                                {!! HTML::link(action('Users\FacturasV33Controller@getXml', [ 'id' => $factura->getId() ]), '<i class="icon-file-xml" aria-hidden="true"></i>', [ 'class' => 'btn btn-default', 'data-toggle' => 'tooltip', 'data-placement' => 'bottom', 'data-original-title' => 'XML Download' ]) !!}
                                {!! HTML::link(action('Users\FacturasV33Controller@getPdf', [ 'id' => $factura->getId() ]), '<i class="icon-file-pdf" aria-hidden="true"></i>', [ 'class' => 'btn btn-default', 'data-toggle' => 'tooltip', 'data-placement' => 'bottom', 'data-original-title' => 'PDF Download' ]) !!}
                                {!! HTML::link(action('Users\FacturasV33Controller@getInvoice', [ 'id' => $factura->getId() ]), '<i class="icon-file-eye" aria-hidden="true"></i>', [ 'class' => 'btn btn-default', 'data-toggle' => 'tooltip', 'data-placement' => 'bottom', 'data-original-title' => 'VER' ]) !!}
                                @if ($factura->getEstatus() === \App\Models\Facturas::TIMBRADA)
                                    {!! HTML::link(action('Users\FacturasV33Controller@getCancelar', [ 'id' => $factura->getId() ]), '<i class="icon-cross" aria-hidden="true"></i>', [ 'class' => 'btn btn-danger eliminar', 'data-toggle' => 'tooltip', 'data-placement' => 'bottom', 'data-original-title' => 'CANCELAR' , 'data-title' => 'Cancelación de Factura', 'data-content' => 'Esta seguro de Cancelar la factura?' ]) !!}
                                @endif
                            </td>
                        </tr>
                    @endforeach
