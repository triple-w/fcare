...
class GenerateEntitiesCommand extends \LaravelDoctrine\ORM\Console\Command
{
    /**
     * The name and signature of the console command.
     * @var string
     */
    protected $signature = 'doctrine:generate:entitieslinux
    ...
    public function fire(ManagerRegistry $registry)
    {
        ...
        foreach ($names as $name) {
            ...
            if (count($metadatas)) {
                ...
                $entityGenerator->setNumSpaces(empty($this->option('num-spaces')) ? 4 : $this->option('num-spaces'));
                ...
                foreach ($metadatas as $key => $metadata) {
                    $metadatas[$key]->name = lcfirst($metadatas[$key]->name);
                    $this->comment(
